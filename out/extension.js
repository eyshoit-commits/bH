...
export function activate(context) {
    const output = vscode.window.createOutputChannel('proxy');
    context.subscriptions.push(output);
    const statusItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 100);
    statusItem.command = 'proxy.showServerControls';
    context.subscriptions.push(statusItem);
    // Status Bar & Notifications
    const updateStatusBar = async () => {
        if (!gateway) {
            statusItem.text = '$(circle-slash) proxy: OFF';
...
*Tip: Enable auto-start in settings for convenience*
    ...
    const getGateway = async () => {
        ...
        try {
            ...
            const skillsPath = skillsConfig.get('path', '.vscode/skills');
            ...
            if (status.running && !wasRunning) {
                ...
                if (config.get('showNotifications', true)) {
                    ...
                    const displayHost = (status.config.host === '0.0.0.0' && status.networkInfo?.localIPs?.length)
                        ...
                        : status.config.host;
                    const protocol = status.isHttps ? 'https' : 'http';
                    const selection = await vscode.window.showInformationMessage(`proxy started at ${protocol}://${displayHost}:${status.config.port}`, 'Open Dashboard');
                    if (selection === 'Open Dashboard') {
                        void vscode.commands.executeCommand('proxy.openDashboard');
                    }
                }
            }
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(CopilotPanel.viewType, provider));
    const showServerControls = vscode.commands.registerCommand('proxy.showServerControls', async () => {
        ...
        if (selection.label.includes('Stop Server')) {
            ...
            const allModels = await selectChatModelsSafe(output);
            const builtInItems = allModels.map(m => ({
            ...
            const customItems = (status.config.customProviders || []).flatMap(provider => (provider.enabled === false ? [] : (provider.models || []).map(modelId => {
                const fullId = `custom/${provider.name}/${modelId}`;
                return {
                    label: fullId === status.config.defaultModel ? `$(check) ${fullId}` : `     ${fullId}`,
                    description: fullId === status.config.defaultModel ? `(current) · custom provider` : `custom provider · ${provider.name}`,
                    modelId: fullId
                };
            })));
            const modelItems = [...builtInItems, ...customItems];
            if (modelItems.length === 0) {
                void vscode.window.showWarningMessage('No models are currently available. Check your VS Code model providers or custom providers.');
                return;
            }
    ...
    if (enabled || autoStart) {
        ...
        getGateway().then(gw => {
            return gw.start().catch(error => {
                ...
                void vscode.window.showErrorMessage(`Failed to start proxy: ${getErrorMessage(error)}`);
            });
        });
    }
    const openChatCommand = vscode.commands.registerCommand('proxy.openCopilotChat', async () => {
        if (!await ensureCopilotChatReady()) {
            return;
        }
        await vscode.commands.executeCommand('workbench.action.chat.open');
    });
    const askChatCommand = vscode.commands.registerCommand('proxy.askCopilot', async (rawPrompt) => {
        if (!await ensureCopilotChatReady()) {
            return;
        }
    ...
    const askSelectionCommand = vscode.commands.registerTextEditorCommand('proxy.askSelectionWithCopilot', async (editor, _edit, rawPrompt) => {
        if (!await ensureCopilotChatReady()) {
            return;
        }
    ...
    const openDashboard = vscode.commands.registerCommand('proxy.openDashboard', () => {
        CopilotPanel.createOrShow(context.extensionUri, getGateway);
    });
    const createShortcutCommand = vscode.commands.registerCommand('proxy.createDesktopShortcut', async () => {
        await createDesktopShortcut();
    });
    const openSettingsCommand = vscode.commands.registerCommand('proxy.openSettings', async () => {
        await vscode.commands.executeCommand('workbench.action.openSettingsJson');
    });
    // URI Handler for deep linking
    // Handles vscode://proxy.proxy/<path>
        ...
        handleUri(uri) {
            ...
            if (path === '/dashboard') {
                ...
                void getGateway().then(gw => gw.startServer().then(() => {
                    void vscode.window.showInformationMessage('proxy server started via shortcut');
                }));
            }
            else if (path === '/stop') {
                ...
                if (gateway) {
                    void gateway.stopServer().then(() => {
                        void vscode.window.showInformationMessage('proxy server stopped via shortcut');
                    });
                }
            }
            else {
...
`.trimStart();
    const editSystemPrompt = vscode.commands.registerCommand('proxy.editSystemPrompt', async () => {
        ...
        const config = vscode.workspace.getConfiguration('githubCopilotApi.server');
        const currentPrompt = config.get('defaultSystemPrompt', '');
    ...
    context.subscriptions.push(openChatCommand, askChatCommand, askSelectionCommand, createShortcutCommand, openSettingsCommand, openDashboard, showServerControls, editSystemPrompt);
}

...
import { buildResponsesCustomProviderChatPayload, extractChatCompletionTextContent, mapChatCompletionToResponsesApiResponse, } from './responsesCustomProviderBridge.js';
...
test('injects the default system prompt when instructions are omitted', () => {
    const payload = buildResponsesCustomProviderChatPayload({
        model: 'custom/local-ollama-proxy/qwen2.5-coder-0.5b-instruct',
        input: 'Hello'
    }, 'qwen2.5-coder-0.5b-instruct', 'Use terse answers.');
...
test('extracts text from structured chat-completion content arrays', () => {
    assert.equal(extractChatCompletionTextContent([
        { type: 'output_text', text: 'Hello' },
        { type: 'output_text', text: ' world' }
    ]), 'Hello world');
});
//# sourceMappingURL=responsesCustomProviderBridge.test.js.map
...
export class CopilotPanel {
        ...
        webviewView.webview.onDidReceiveMessage(async (data) => {
            ...
            switch (data.type) {
                ...
                case 'editSystemPrompt':
                    void vscode.commands.executeCommand('proxy.editSystemPrompt');
                    break;
                case 'switchModel':
    ...
    static async createOrShow(extensionUri, gatewayAccessor, scrollTarget) {
        ...
        const panel = vscode.window.createWebviewPanel('proxyDashboard', 'proxy Dashboard', column, {
            enableScripts: true,
        ...
        }, undefined, CopilotPanel.panelDisposables);
    ...
    static async openWiki(extensionUri, gateway) {
        ...
        const panel = vscode.window.createWebviewPanel('proxyWiki', '📚 Proxy Guide', column, {
            enableScripts: true,
            retainContextWhenHidden: true,
            localResourceRoots: [extensionUri]
        });
...
<body>
    ...
    <p class="muted">Complete reference for connecting to the proxy gateway from various languages.</p>
...
for await (const chunk of stream) {
  ...
  "githubCopilotApi.mcp.servers": {
    ...
    static handleMessage(data, gateway) {
        switch (data.type) {
            case 'openChat':
                void vscode.commands.executeCommand('proxy.openCopilotChat');
                break;
            case 'askCopilot':
                void vscode.commands.executeCommand('proxy.askCopilot');
                break;
            case 'showControls':
                void vscode.commands.executeCommand('proxy.showServerControls');
                break;
            case 'openSettings':
                void vscode.commands.executeCommand('proxy.openSettings');
                break;
            case 'startServer':
            ...
            case 'saveCustomProvider':
                if (data.value && typeof data.value === 'object') {
                    void (async () => {
                        const nextProvider = data.value;
                        const providers = gateway.getCustomProviders().filter(provider => provider.name !== nextProvider.name);
                        providers.push(nextProvider);
                        await gateway.setCustomProviders(providers);
                        if (CopilotPanel.currentPanel) {
                            CopilotPanel.currentPanel.webview.html = await CopilotPanel.getPanelHtml(CopilotPanel.currentPanel.webview, gateway);
                        }
                    })();
            ...
            case 'deleteCustomProvider':
                if (typeof data.value === 'string') {
                    void (async () => {
                        const providers = gateway.getCustomProviders().filter(provider => provider.name !== data.value);
                        await gateway.setCustomProviders(providers);
                        if (CopilotPanel.currentPanel) {
                            CopilotPanel.currentPanel.webview.html = await CopilotPanel.getPanelHtml(CopilotPanel.currentPanel.webview, gateway);
                        }
            ...
            case 'fetchCustomProviderModels':
                if (data.value && typeof data.value === 'object' && CopilotPanel.currentPanel) {
                    void (async () => {
                        try {
                            const provider = data.value;
                            const models = await gateway.fetchCustomProviderModels(provider);
                            ...
                            void CopilotPanel.currentPanel?.webview.postMessage({
                                type: 'customProviderModelsError',
                                providerName: data.value.name,
                ...
                break;
            case 'testCustomProvider':
                if (data.value && typeof data.value === 'object' && CopilotPanel.currentPanel) {
                    void (async () => {
                        try {
                            await gateway.testCustomProvider(data.value);
                            void CopilotPanel.currentPanel?.webview.postMessage({
...
<body>
    <div class="page">
        <div class="hero">
            <div>
                ...
                <p>Monitor and control your local multi-provider gateway.</p>
            ...
            <div style="display: flex; align-items: flex-start; gap: 12px; margin-bottom: 16px;">
                ...
                <div>
                    ...
                    <div class="muted" style="font-size: 12px; line-height: 1.5;">
                        Access your proxy endpoint from anywhere — your phone, tablet, another computer, or share with friends. 
                ...
                <div class="stacked">
                    ...
                    <div style="margin-top: 16px; border-top: 1px solid var(--vscode-widget-border); padding-top: 16px;">
                        ...
                        <div class="pill-row" id="ip-list">
                            ${(config.ipAllowlist || []).map(ip => `<span class="pill" style="font-size: 11px; display: flex; align-items: center; gap: 4px;">${ip} <span class="btn-remove-ip" data-value="${ip}" style="cursor: pointer; opacity: 0.6;">×</span></span>`).join('')}
        ...
        <div class="card full-width">
            ...
            <p class="muted" style="margin-bottom: 12px;">
                Add OpenAI-compatible endpoints like local Ollama bridges, Codex-style servers, or NVIDIA NIM. These models are merged into <code>/v1/models</code>.
            </p>

            <div style="display: grid; gap: 12px; margin-bottom: 16px;">
                ${customProviders.map(provider => `
                    <div style="border: 1px solid var(--vscode-widget-border); border-radius: 8px; padding: 12px; background: var(--vscode-textBlockQuote-background);">
                        <div style="display: flex; justify-content: space-between; gap: 12px; align-items: flex-start; flex-wrap: wrap;">
                            <div>
                                <div style="font-weight: 600;">${provider.name}</div>
                                <div class="muted" style="font-size: 12px; margin-top: 4px;"><code>${provider.baseUrl}</code></div>
                                <div class="muted" style="font-size: 11px; margin-top: 6px;">
                                    Models: ${(provider.models || []).join(', ') || 'auto-discover'}
                                </div>
                                <div class="muted" style="font-size: 11px; margin-top: 4px;">
                                    Auto discover models: ${provider.autoDiscoverModels === false ? 'No' : 'Yes'}
                                </div>
                            </div>
                            <div style="display: flex; gap: 8px; flex-wrap: wrap;">
                                <button class="secondary btn-test-provider" data-provider='${JSON.stringify(provider).replace(/'/g, '&apos;')}' style="width: auto;">🧪 Test</button>
                                <button class="secondary btn-fetch-provider-models" data-provider='${JSON.stringify(provider).replace(/'/g, '&apos;')}' style="width: auto;">📥 Fetch Models</button>
                                <button class="secondary btn-delete-provider" data-name="${provider.name}" style="width: auto;">🗑 Remove</button>
                            </div>
                        </div>
                    </div>
                `).join('')}
                ${customProviders.length === 0 ? '<div class="muted" style="padding: 12px; text-align: center; border: 1px dashed var(--vscode-widget-border); border-radius: 8px;">No custom providers configured yet.</div>' : ''}
            </div>

            <div style="padding: 12px; border-radius: 8px; background: var(--vscode-textBlockQuote-background); display: grid; gap: 10px;">
                <div style="font-size: 11px; font-weight: 600; opacity: 0.8;">ADD OR UPDATE PROVIDER</div>
                <div style="display: grid; gap: 8px;">
                    <input type="text" id="custom-provider-name" placeholder="Provider name, e.g. local-openai">
                    <input type="text" id="custom-provider-base-url" placeholder="Base URL, e.g. http://127.0.0.1:1234/v1">
                    <input type="text" id="custom-provider-api-key" placeholder="Optional API key or \${env:NAME}">
                    <input type="text" id="custom-provider-headers" placeholder='Optional headers JSON, e.g. {"X-Test":"1"}'>
                    <label style="font-size: 11px; display: flex; align-items: center; gap: 6px;">
                        <input type="checkbox" id="custom-provider-auto-discover" checked style="width: 14px; height: 14px;">
                        Automatically fetch models from the provider (/v1/models)
                    </label>
                    <textarea id="custom-provider-models" placeholder="One model per line or comma-separated" rows="4" style="resize: vertical;"></textarea>
                </div>
                <div style="display: flex; gap: 8px; flex-wrap: wrap; justify-content: flex-end;">
                    <button class="secondary" id="btn-test-provider-draft" style="width: auto;">🧪 Test Draft</button>
                    <button class="secondary" id="btn-fetch-provider-models-draft" style="width: auto;">📥 Fetch Into Form</button>
                    <button id="btn-save-provider" style="width: auto;">💾 Save Provider</button>
                </div>
                <div id="custom-provider-status" class="muted" style="font-size: 12px;"></div>
            </div>
        </div>
        ...
        <div class="card full-width">
            ...
            <div style="margin-bottom: 20px;">
                ...
                <div id="builtin-patterns-list" style="display: flex; flex-direction: column; gap: 8px;">
                    ${(config.redactionPatterns || []).filter((p) => p.isBuiltin).map((p) => `
    ...
    <script nonce="${nonce}">
        ...
        function setCustomProviderStatus(message, isError) {
            var el = document.getElementById('custom-provider-status');
            if (!el) return;
            el.textContent = message || '';
            el.style.color = isError ? 'var(--vscode-testing-iconFailed)' : 'var(--vscode-testing-iconPassed)';
        }
        ...
        function getDraftProvider() {
            var name = document.getElementById('custom-provider-name').value.trim();
            var baseUrl = document.getElementById('custom-provider-base-url').value.trim();
            var apiKey = document.getElementById('custom-provider-api-key').value.trim();
            var autoDiscover = document.getElementById('custom-provider-auto-discover').checked;
            var models = parseProviderModels(document.getElementById('custom-provider-models').value);
            var headers = parseProviderHeaders(document.getElementById('custom-provider-headers').value);
            return {
                name: name,
                baseUrl: baseUrl,
                apiKey: apiKey,
                ...
                headers: headers,
                enabled: true
                , autoDiscoverModels: autoDiscover
            };
        }

        function fillDraftProvider(provider) {
            document.getElementById('custom-provider-name').value = provider.name || '';
            document.getElementById('custom-provider-base-url').value = provider.baseUrl || '';
            document.getElementById('custom-provider-api-key').value = provider.apiKey || '';
            document.getElementById('custom-provider-headers').value = provider.headers && Object.keys(provider.headers).length ? JSON.stringify(provider.headers) : '';
            document.getElementById('custom-provider-models').value = (provider.models || []).join('\n');
            document.getElementById('custom-provider-auto-discover').checked = provider.autoDiscoverModels !== false;
        }
        ...
        var btnSaveProvider = document.getElementById('btn-save-provider');
        if (btnSaveProvider) {
            btnSaveProvider.onclick = function() {
                try {
                    var provider = getDraftProvider();
                    ...
                    setCustomProviderStatus('Provider saved. Dashboard will refresh.', false);
                } catch (e) {
                    setCustomProviderStatus('Invalid provider data: ' + e.message, true);
                }
            };
        }

        var btnFetchDraftModels = document.getElementById('btn-fetch-provider-models-draft');
        if (btnFetchDraftModels) {
            btnFetchDraftModels.onclick = function() {
                try {
                    var provider = getDraftProvider();
                    if (!provider.name || !provider.baseUrl) {
                        setCustomProviderStatus('Enter a provider name and Base URL first.', true);
                        return;
                    }
                    setCustomProviderStatus('Fetching models...', false);
                    vscode.postMessage({ type: 'fetchCustomProviderModels', value: provider });
                } catch (e) {
                    setCustomProviderStatus('Invalid headers JSON: ' + e.message, true);
                }
            };
        }

        var btnTestDraft = document.getElementById('btn-test-provider-draft');
        if (btnTestDraft) {
            btnTestDraft.onclick = function() {
                try {
                    var provider = getDraftProvider();
                    if (!provider.name || !provider.baseUrl) {
                        setCustomProviderStatus('Enter a provider name and Base URL first.', true);
                    ...
                    setCustomProviderStatus('Testing provider...', false);
                    vscode.postMessage({ type: 'testCustomProvider', value: provider });
                } catch (e) {
                    setCustomProviderStatus('Invalid headers JSON: ' + e.message, true);
                }
            };
        ...
        document.querySelectorAll('.btn-delete-provider').forEach(function(btn) {
            btn.onclick = function() {
                ...
                if (name && confirm('Remove provider "' + name + '"?')) {
                    vscode.postMessage({ type: 'deleteCustomProvider', value: name });
                }
            };
        });

        document.querySelectorAll('.btn-fetch-provider-models').forEach(function(btn) {
            btn.onclick = function() {
                try {
                    var provider = JSON.parse(btn.getAttribute('data-provider').replaceAll('&apos;', "'"));
                    fillDraftProvider(provider);
                    setCustomProviderStatus('Fetching models for ' + provider.name + '...', false);
                    vscode.postMessage({ type: 'fetchCustomProviderModels', value: provider });
                } catch (e) {
                    setCustomProviderStatus('Could not read provider payload.', true);
                }
            };
        });
        ...
        document.querySelectorAll('.btn-test-provider').forEach(function(btn) {
            btn.onclick = function() {
                try {
                    var provider = JSON.parse(btn.getAttribute('data-provider').replaceAll('&apos;', "'"));
                    fillDraftProvider(provider);
                    setCustomProviderStatus('Testing ' + provider.name + '...', false);
                    vscode.postMessage({ type: 'testCustomProvider', value: provider });
                } catch (e) {
                    setCustomProviderStatus('Could not read provider payload.', true);
                }
        ...
        window.addEventListener('message', function(event) {
            ...
            } else if (message.type === 'customProviderModelsFetched') {
                var models = (message.models || []).map(function(model) { return model.id; });
                document.getElementById('custom-provider-models').value = models.join('\n');
                setCustomProviderStatus('Fetched ' + models.length + ' models from ' + message.providerName + '.', false);
            } else if (message.type === 'customProviderModelsError') {
                setCustomProviderStatus('Model fetch failed: ' + message.error, true);
            } else if (message.type === 'customProviderTestResult') {
                if (message.ok) {
                    setCustomProviderStatus('Provider ' + message.providerName + ' is reachable.', false);
                } else {
                    setCustomProviderStatus('Provider test failed: ' + message.error, true);
                }
            }
        });

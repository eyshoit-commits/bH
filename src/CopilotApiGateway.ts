...
} from './responsesCustomProviderBridge';
...
export async function selectChatModelsSafe(
	output?: vscode.OutputChannel,
	timeoutMs: number = SELECT_CHAT_MODELS_TIMEOUT_MS
...
interface NormalizedModelRejection {
	provider: string
	raw: any
	normalized: NormalizedModel | null
	reason: 'missing_id' | 'validation_error'
}
...
export class CopilotApiGateway implements vscode.Disposable {
	...
	private readonly customProviderModelCache = new Map<string, { models: string[]; expires: number }>();
	private readonly CUSTOM_PROVIDER_CACHE_TTL_MS = 60_000;
	...
	public async setCustomProviders(providers: CustomProviderConfig[]): Promise<void> {
		await this.updateServerConfig({ customProviders: providers });
	}

	public async fetchCustomProviderModels(provider: CustomProviderConfig): Promise<CustomProviderModelInfo[]> {
		...
		return models
			...
			.filter((model): model is CustomProviderModelInfo => model !== null);
	}

	public async testCustomProvider(provider: CustomProviderConfig): Promise<void> {
	...
	private async getAvailableModels(): Promise<NormalizedModel[]> {
		const rejections: NormalizedModelRejection[] = [];
	...
	private async normalizeVsCodeModels(rejections: NormalizedModelRejection[]): Promise<NormalizedModel[]> {
		const models = await selectChatModelsSafe(this.output);
	...
	private async normalizeCustomProviderModels(rejections: NormalizedModelRejection[]): Promise<NormalizedModel[]> {
	...
	private normalizeModelList<T>(items: T[], converter: (item: T) => { raw: any; provider: string; options?: NormalizeModelOptions }, rejections: NormalizedModelRejection[]): NormalizedModel[] {
	...
	private logRejectedModels(rejections: NormalizedModelRejection[]): void {
	...
	private truncateForLog(value: unknown, maxLength = 400): string {
		try {
	...
	private splitPrefixedModelId(modelId: string): { provider?: string; rawId: string } {
		if (!modelId) {
			return { rawId: '' };
		}
	...
	private buildCustomProviderModelId(providerName: string, modelId: string): string {
		return `custom/${this.slugifyProviderName(providerName)}/${modelId}`;
	}

	private slugifyProviderName(value: string): string {
		return value
			.trim()
			.toLowerCase()
			.replace(/[^a-z0-9]+/g, '-')
	...
	private async findCustomProviderModel(requestedModel: string): Promise<CustomProviderModel | null> {
		const models = await this.getCustomProviderModels();
	...
	private async resolveProviderModels(provider: CustomProviderConfig): Promise<string[]> {
		if (!provider.enabled) {
			return [];
		}
	...
	private async getCustomProviderModels(): Promise<CustomProviderModel[]> {
		const entries: CustomProviderModel[] = [];
	...
	private resolveProviderApiKey(provider: CustomProviderConfig): string {
	...
	private getCustomProviderHeaders(provider: CustomProviderConfig): Record<string, string> {
	...
	private async proxyChatCompletion(model: CustomProviderModel, payload: any): Promise<Record<string, unknown>> {
	...
	private async proxyStreamingChatCompletion(model: CustomProviderModel, payload: any, res: ServerResponse): Promise<void> {
		const response = await fetch(`${model.provider.baseUrl}/chat/completions`, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
	...
	private async processCustomProviderResponsesApi(
		...
		return mapChatCompletionToResponsesApiResponse(payload, exposedModel, body as any);
	}

	private async processStreamingCustomProviderResponsesApi(
		model: CustomProviderModel,
		exposedModel: string,
		payload: ResponsesApiRequest,
		req: IncomingMessage,
	...
	private buildResponsesCustomProviderOptions(payload: ResponsesApiRequest): Record<string, unknown> {
...
export async function ensureCopilotChatReady(): Promise<boolean> {
	...
	if (isCursor) {
		const choice = await vscode.window.showWarningMessage(
			'No language models available. Enable models in Cursor settings and ensure you are signed in.',
			'Open Settings',
			'Cancel',
		);
...
function getServerConfig(): ApiServerConfig {
	...
	const rawCustomProviders = configuration.get<unknown[]>('server.customProviders', []);
	const customProviders: CustomProviderConfig[] = rawCustomProviders
...
function parseCustomProviderConfig(raw: unknown): CustomProviderConfig | null {

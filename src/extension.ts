...
*Tip: Enable auto-start in settings for convenience*
			...
			if (status.running && !wasRunning) {
				...
				if (config.get<boolean>('showNotifications', true)) {
					...
					const selection = await vscode.window.showInformationMessage(
						`proxy started at ${protocol}://${displayHost}:${status.config.port}`,
						'Open Dashboard'
					);
	...
	const showServerControls = vscode.commands.registerCommand('proxy.showServerControls', async () => {
		...
		if (selection.label.includes('Stop Server')) {
			...
			const customItems = (status.config.customProviders || []).flatMap(provider =>
				(provider.enabled === false ? [] : (provider.models || []).map(modelId => {
	...
	const askChatCommand = vscode.commands.registerCommand('proxy.askCopilot', async (rawPrompt?: unknown) => {
		if (!await ensureCopilotChatReady()) {
			return;
		}
	...
	const askSelectionCommand = vscode.commands.registerTextEditorCommand('proxy.askSelectionWithCopilot', async (editor, _edit, rawPrompt?: unknown) => {
		if (!await ensureCopilotChatReady()) {
			return;
		}

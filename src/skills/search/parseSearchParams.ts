...
export function parseSearchParams(search: URLSearchParams): SearchParams {
	...
	if (sourceValue) {
		...
		if (!VALID_SOURCES.includes(normalized as CoreSkillSource)) {
			throw new Error(`Invalid source: ${sourceValue}`);
		}
		params.source = normalized as CoreSkillSource;
	}
...
function collectCsv(search: URLSearchParams, key: string): string[] {
...
function parseIntegerParam(value: string | null, key: string, requirePositive: boolean): number | undefined {
	if (!value) {
		return undefined;
	}

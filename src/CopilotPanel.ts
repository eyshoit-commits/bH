...
export class CopilotPanel implements vscode.WebviewViewProvider {
    ...
    private static async getWikiHtml(webview: vscode.Webview, gateway: CopilotApiGateway): Promise<string> {
        ...
        const customProviders = (config.customProviders || []) as CustomProviderConfig[];
    ...
    private static handleMessage(data: { type: string; value?: unknown }, gateway: CopilotApiGateway): void {
        switch (data.type) {
            ...
            case 'saveCustomProvider':
                if (data.value && typeof data.value === 'object') {
                    void (async () => {
                        const nextProvider = data.value as CustomProviderConfig;
            ...
            case 'fetchCustomProviderModels':
                if (data.value && typeof data.value === 'object' && CopilotPanel.currentPanel) {
                    void (async () => {
                        try {
                            const provider = data.value as CustomProviderConfig;
                            ...
                            void CopilotPanel.currentPanel?.webview.postMessage({
                                ...
                                providerName: (data.value as CustomProviderConfig).name,
            ...
            case 'testCustomProvider':
                if (data.value && typeof data.value === 'object' && CopilotPanel.currentPanel) {
                    void (async () => {
                        try {
                            await gateway.testCustomProvider(data.value as CustomProviderConfig);
                            void CopilotPanel.currentPanel?.webview.postMessage({
